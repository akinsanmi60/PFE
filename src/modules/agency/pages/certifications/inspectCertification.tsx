...
function InspectedCertification() {
  ...
  const openFilterBox = () => handleModalOpen('inspectedFilterForm');

  const closeFilterBox = () => {
    handleModalClose('inspectedFilterForm');
  };
      ...
      <div className="w-full bg-primary-white rounded-lg mt-[30px]">
        <CustomTable<ICertification>
          ...
          onRowClick={(row: ICertification) => {
            navigate(
              `/${AgencyUserPath.certificationDetial(row?.id, 'inspected')}`,
            );
          }}
        />
      </div>

      {modalState?.modalType === 'inspectedFilterForm' && (
...
export default InspectedCertification;
