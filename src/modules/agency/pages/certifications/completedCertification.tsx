...
function CompletedCertification() {
  ...
  const openFilterBox = () => handleModalOpen('certifiedFilterForm');

  const closeFilterBox = () => {
    handleModalClose('certifiedFilterForm');
  };
      ...
      <div className="w-full bg-primary-white rounded-lg mt-[30px]">
        <CustomTable<ICertification>
          ...
          onRowClick={(row: ICertification) => {
            navigate(
              `/${AgencyUserPath.certificationDetial(row?.id, 'certified')}`,
            );
          }}
        />
      </div>

      {modalState?.modalType === 'certifiedFilterForm' && (
